...
public class ExplorerFirstAttempt {
    ...
    private ArrayList<List<Node>> escapePlan = new ArrayList<>();
...
/*
    public Long earlierVisitInStack(ExplorationState state){
        List<Long> stackList = stackToList();
        for(int i = 0; i < stackList.size()-1; i++){
            for (NodeStatus neighbour : closestNeighbour(state)) {
                if(stackList.contains(neighbour.getId())){
...
*/
    ...
    public void escape(EscapeState state) {
        ...
        System.out.println("");
        /*do{
            state.moveTo(state.getVertices().iterator().next());
        }while(state.getCurrentNode() != state.getExit());*/
        List<Node> solutionList = new ArrayList<>();
        System.out.println(listSolutions(state.getCurrentNode(), state, solutionList, 0).toString());
        escapePlan.get(0).forEach(state::moveTo);

    }

    public List<Node> listSolutions(Node currentNode, EscapeState state, List<Node> solution, int i){
        while (currentNode != state.getExit()) {
            for (Node neighbour : currentNode.getNeighbours()) {
                solution.add(neighbour);
                if(i > 200) { solution.clear(); return solution; }
                listSolutions(neighbour, state, solution, (i+1));
            }
        }
        escapePlan.add(solution);
        return solution;
    }
}
